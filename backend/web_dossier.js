...
(async () => {
  ...
  if (!seed) { console.error('usage: node web_dossier.js "ENTITY NAME"'); process.exit(1); }
  if (!serp.available()) { console.error('SERP_API_KEY not set'); process.exit(1); }
  ...
  const queriesFor = (term, kind) => kind === 'person'
    ...
    : [`"${term}" ${LOCALITY} owner principal`, `"${term}" ${LOCALITY}`];
  ...
  while (frontier.length && searches < MAX_SEARCHES) {
    ...
    for (const r of results) {
      ...
      for (const p of peopleFrom(title, snip)) {
        ...
        let e = people.get(k); if (!e) { e = { name: p.name, roles: new Set(), sources: new Set(), given: p.given }; people.set(k, e); }
        ...
        if (p.given) enqueue(p.name, 'person');
      }
      ...
      for (const m of snip.matchAll(PHONE_RX)) contacts.phones.set(m[0].replace(/\s+/g, ' ').trim(), host);
  ...
  // cross-reference operator entities against our delinquent roll
  ...
  // ---- Motivation read ----
  const topPerson = [...people.values()].filter(p => p.given).sort((a, b) => b.sources.size - a.sources.size)[0];
  ...
  [...people.values()].filter(p => p.given).sort((a, b) => b.sources.size - a.sources.size)
    .forEach(p => console.log(`  • ${p.name} — ${S(p.roles) || 'role?'}   [${S(p.sources)}]`));
  const maybe = [...people.values()].filter(p => !p.given);
  ...
  if (bits.length) {
    ...
    if (seedOwed && (recentT.length || capitalHeavy.length))
      console.log(`  → Likely over-extended on newer/capital-heavy projects; this delinquent parcel is a candidate to LIQUIDATE for cash. Strong motivated-seller angle.`);
